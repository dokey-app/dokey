...
// Потолок — десятичное целое без знака, в кавычках или без, — и выражение `${{ … }}`,
// разворачиваемое в число на прогоне. Числа мало: число должно подойти и потребителю, а
// документация GitHub требует от `timeout-minutes` целого («Fractional values are not supported.
// `timeout-minutes` must be a positive integer»), поэтому дробное (`5.5`, `5.0`, `"5.5"`) — не
// потолок, хотя число из него выходит (T-252). Прочие числовые формы YAML (`+5`, `5e1`, `0x10`,
// `0o12`, `.5`, `5.`) число тоже дают, но здесь красные сознательно: расхождения с настоящим
// разбором держатся в сторону ложной красноты, а не молчания. Пустое значение — не одна форма, а
// семь: `null`, `Null`, `NULL`, `~`, `''`, `""` и вовсе ничего дают у js-yaml 4.3.2 `null` или
// пустую строку. Перечень непригодных форм закрывал бы найденное, а не класс, поэтому список
// белый: всё, что под него не подошло (`abc`, `5m`, `false`, `[]`, `-5`), — не потолок.
function isCeiling(value: string): boolean {
  const scalar = /^(["'])(.*)\1$/.exec(value)?.[2] ?? value;
  return /^\d+$/.test(scalar) || /^\$\{\{.+\}\}$/.test(scalar);
}
...
describe('потолок времени job в workflow', () => {
  ...
  // на всех этих формах js-yaml видит потолок, значит видит и проверка. `0` — целое, и потолок он
  // даёт: job с ним не висит шесть часов умолчания, а обрывается сразу. Документация требует
  // положительного целого, так что осмысленность `0` — предмет ревью значения, а не этой
  // проверки: предмет проверки — прогон, оставшийся без потолка вовсе.
  ...
  // класс «десятичное целое без знака»: всё прочее красное сознательно, потому что ложная
  // краснота останавливает автора строки, а ложная зелень — никого.
  it('экзотическая числовая форма — не потолок', () => {
    for (const value of ['+5', '5e1', '1e1', '0x10', '0o12']) {
      const text = [
        'jobs:',
        '  lint:',
        `    timeout-minutes: ${value}`,
