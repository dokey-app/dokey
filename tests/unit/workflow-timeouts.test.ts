...
// Потолок — десятичное число: целое или дробное, в кавычках или без, — и выражение `${{ … }}`,
// разворачиваемое в число на прогоне. Прочие числовые формы YAML (`+5`, `5e1`, `0x10`, `0o12`,
// `.5`, `5.`) число тоже дают, но здесь красные сознательно: расхождения с настоящим разбором
// держатся в сторону ложной красноты, а не молчания. Пустое значение — не одна форма, а семь:
// `null`, `Null`, `NULL`, `~`, `''`, `""` и вовсе ничего дают у js-yaml 4.3.2 `null` или пустую
// строку. Перечень пустых форм закрывал бы найденное, а не класс, поэтому список белый: всё, что
// под него не подошло (`abc`, `5m`, `false`, `[]`, `-5`), — не потолок.
