...
// Переменная окружения переопределяет бюджет: сегодняшний репозиторий (341 путь, ≈ 9,5 тыс.
// символов) целиком влезает в одну пачку, и без неё разбиение недостижимо — а разбиение,
// которое ничем не проверить, молча ломается ровно в тот день, когда репозиторий дорастёт.
const ARGV_BUDGET = Number(process.env.DOKEY_ARGV_BUDGET) || 24_000;
