...
**Шаг 3 — коммит.** Строка остатка: файл `docs/tasks.md`. Отметку `- [x] T-013` ставит ship.
...
- [x] verify — `pnpm test:unit` (`docs-structure.test.ts` держит форму документа) и чек-лист
      ...
      - [ ] T-256 — закрыть `set:html` в `src/**`: правило линта T-013 держит SEC-21 только в JS
        ...
        удаление краснит `pnpm test:unit`
...
- **Шапка `pr.yml`: «эти две строки с датой» → «датированный факт».** Датированный факт занимает
  строки 2–5 шестистрочного комментария, а «Done when» говорит о нём в единственном числе. Счёт
  строк из формулировки убран — и в шапке, и здесь.
