...
- [x] Приёмка (диффа не даёт): `gh run view <id прогона PR этой задачи> --json jobs --jq '.jobs[] | "\(.name) \(.conclusion)"'`
      ...
      Отметка здесь стоит по прогону PR #27 (`t-255-audit-values`, тот же файл без правок job'ов);
      прогоном **этой** задачи она закрывается на этапе ship — PR ещё не создан, и раньше ship
      его номера не существует.
