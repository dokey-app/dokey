...
**Опирается на:** [glossary.md](glossary.md) (ENT-01…ENT-32 включая ENT-21a, ROLE-01…ROLE-06, §3 глаголы),
...
| **К-4 · Аналитика** | Umami на своём VPS за прокси на своём origin `dokey.app/s/event` | **12 месяцев** (D-113): удаление ежемесячным заданием, число публикуется на SCR-10. Дамп базы шифруется до записи на носитель и живёт не дольше записей в базе (D-169) | ROLE-03 | ENT-28, ENT-29 |
...
Покрывает все ENT-01…ENT-32, включая ENT-21a (ENT-31 и ENT-32 заведены D-153). Границы контуров показаны комментариями. Сущности без
...
| `detectorStep` | `{ step, recognizer, meaningfulWhen }` | нет | Ступень REQ-INPUT-01, которую обслуживает инструмент; `step` уникальна в пределах сборки. **`recognizer` линеен по построению** (D-168): в его регулярках нет вложенных квантификаторов, альтернатив, совпадающих с одним текстом несколькими способами, и обратных ссылок. Проверка — в RUN-01: у ступени враждебный образец в корпусе, и время на входе размера 4n превышает время на n не более чем в 8 раз. Ступень принадлежит записи реестра, а не константе определителя: при неполном каталоге (жёсткий минимум M-02) отсутствующая ступень не активна, не выигрывает ранжирование и не показывается чипсом — D-49 п. 3. **`meaningfulWhen` — предикат осмысленности, объявленный рядом с распознавателем** (D-73 п. 3): именно он даёт `TypeCandidate.meaningful`, и без него REQ-INPUT-01 не имеет признака победы. Базовая редакция девяти предикатов — таблица D-73 п. 3. Гейт CI: ни одна ступень не ссылается на инструмент вне реестра сборки |
...
#### ToolLanding (ENT-06) и FaqItem (ENT-32)
...
| Поле | Тип | Обяз. | Ограничения и источник |
|---|---|---|---|
| `id` | `string` (slug) | да | **Страниц пять, считая корень, и один раздел** (D-82 п. 1, D-91): «Как это проверить», «Держать под рукой», бенчмарк TTR, «Развёртывание в своей сети»; «Об обязательствах» остаётся разделом внутри развёртывания (REQ-CONTENT-05), а не пятой страницей |
...
| `description` | `string` | да | Содержит категорию (D-20 п. 4). Непуст и уникален по всей сборке — тот же гейт CI, что у лендингов: G-06 проверяет каждый HTML сборки (D-160) |
...
| `platformClass` | `enum {desktop, mobile}` | да | Часть ключа: на мобильных устройствах пороги ниже, при этом 0 сценариев падает без сообщения (NFR-14). Класс — устройство, а не браузер: определяется один раз при старте оболочки по `matchMedia('(pointer: coarse)')`, UA не читается (D-161) |
...
| `source` | `enum {measured, inherited, emulated}` | да | `measured` — получено процедурой D-51 на настоящем движке класса; `inherited` — единый порог оболочки на размер поля ввода у инструментов, где предел недостижим на разумных вводах (URL, Timestamp, UUID; D-51 п. 7); **`emulated` — получено на эмуляции движка** (D-142: устройства у проекта нет). Все числа класса `mobile` меряются на эмуляции мобильного WebKit — самого ограниченного (D-51 п. 3) — и помечаются `source = emulated` (D-161). Пустой записи не бывает ни у одного инструмента |
...
**Оговорка о происхождении печатается рядом с числом** (D-142): порог с `source = emulated` публикуется
с пометкой о том, что он замерен на эмуляции движка, — так же, как `provisional` помечает
предварительные. Читатель документации инструмента видит, откуда взялось обещание.
...
`platformClass` вычислен один раз при старте оболочки (D-161).
...
| `expectedStep` | `enum` ступени лестницы | да | Ожидаемая ступень ENT-19 (D-83 п. 1) по **полному** каталогу. Обязана совпадать с головой `expectedChips` — сверка бесплатная и ловит расхождение разметки (D-109) |
| `expectedChips` | `enum[]` ступеней | да | **Ожидаемый состав и порядок чипсов** (D-73 п. 3, D-66). У неоднозначного образца размечается именно порядок, а не единственный ответ: иначе фикстура проверяет вкус разметчика, а не механизм **Размечается один раз по полному каталогу (D-109);** прогон пересекает ожидание с активными ступенями сборки — при жёстком минимуме M-02 выпавшие ступени вычёркиваются, порядок оставшихся сохраняется. Второго поля на каждый состав сборки не заводится: ожидание одно, производных — сколько сборок |
...
#### ToolContract (ENT-25) и ContractClause (ENT-31)
...
Состав P0a — одиннадцать пунктов: `computable-in-browser`, `text-or-small-file`, `seconds-not-minutes`,
...
`registry-entry` (REQ-CORE-01), `no-maintenance-promise` (D-37), `text-only-dom` (D-129: пользовательские данные попадают в DOM только через `textContent` и создание узлов).

**Связи.** `ToolContract (1) — (11) ContractClause`; `ContractClause (0..N) — (0..1) Tool` в момент
приёма позиции.
...
**Различия состава артефакта между профилями — перечень исчерпывающий** (D-163):

| Что | `public` | `selfhosted` |
|---|---|---|
| Отправитель событий и ЭП-09 | Есть | Физически отсутствуют (ИНВ-11) |
| `sitemap.xml` | Есть, совпадает с реестром (G-07) | Отсутствует: краулера во внутреннем контуре нет, а адреса `dokey.app` в образе делали бы документ невалидным |
| Состав `robots.txt` | Выводится из признака индексации сборки (D-164) | `Disallow: /` без `Sitemap` (D-164) |

Иных различий состава нет; новое различие заводится только решением. Различия заголовков — за
D-150. Реестр остаётся единственным источником sitemap (ИНВ-06): профиль решает, выпускать ли
производную.
...
| `status` | `enum {example, empty, held, pending, counted, ready, failed, refused}` | да | Жизненный цикл — §3.2 |
...
| `failureReason` | `enum {type_not_detected, limit_exceeded, parse_error, cancelled, tool_not_found, tool_unavailable}` | нет | Обязателен при `status ∈ {failed, refused}`. Шесть категорий REQ-METR-02 (D-132, D-133); значение уходит наружу и потому входит в закрытый список ENT-29. **Написание `limit_exceeded` — единственное:** до 2026-09-09 эта строка называла категорию `threshold_exceeded`, расходясь с analytics-spec §3 и формулой CB-06 |
...
D-71: заводился закрытый список служебных ключей, и в P0a в нём был ровно один — показана ли
подсказка об активации ярлыка. **D-124 закрыл список пустым:** ключ переехал в память оболочки,
REQ-ENTRY-05 читается как «один раз за сессию вкладки», и в устройстве не остаётся ничего, кроме
precache. Персистентность продукта равна нулю без единого исключения.
...
| Поле | Тип | Обяз. | Ограничения и источник |
|---|---|---|---|
| `name` | `string` | да | Имена закреплены реестром analytics-spec §3 — пять событий EVT-01…EVT-05 (D-120), корень глагола — глоссарий §3; закрытое множество имён проверяет G-05 (D-136) |
...
которым содержимое восстановимо (REQ-TRUST-06, PRD §6). **Идентификатора сайта тоже нет:**
`website`, которого требует приёмник, подставляет прокси на edge (D-104), поэтому исходящая
запись остаётся семью полями. Идентификатора человека тоже нет: Umami
различает посетителя псевдонимом, который **прокси считает на edge** из IP и **соли с суточной
ротацией** (D-96); самого адреса приёмник не получает. Раньше хеш считался на
...
ROLE-03 — десять прокси-метрик PM-01…PM-10, все агрегатные, все помесячные или пореализные. Ни одна
не требует выборки по отдельному визиту, и это не совпадение: выборка по визиту — первый шаг к
идентификации, отменённой D-05. **D-127 сделал это запретом:** выборка по одному визиту или одному посетителю не делается, в том числе ради отладки, а панель приёмника доступна только через SSH-туннель. Единицы счёта заданы D-55: **вход** — доставленное событие
...
| Поле | Тип | Обяз. | Ограничения и источник |
|---|---|---|---|
| `field` | `string` | да | Имя разрешённого поля события; ровно семь записей (PRD §6, D-124) |
...
| Прочитать метрику PM-01…PM-10 | К-4 | Индексы Umami; продукт их не проектирует | Не в бюджете продукта |
...
| ENT-23 `RouteTeardown` | Обнуление при уходе с маршрута | Действие, а не запись. Состав обнуляемого — семь пунктов раздела «teardown» контракта ENT-25 (D-48). **Триггер — событие свопа `astro:before-swap`, а не выгрузка документа** (D-105): модульный скрипт исполняется один раз за жизнь документа, поэтому состояние модуля переход переживает, и обнуление обязано быть явным. Его «состояние» — факт, что после ухода поля ENT-14 и ENT-15 пусты; проверяется автотестом NFR-10, а не полем. По потере фокуса вкладки не срабатывает сознательно |
| ENT-30 `HandoffBuffer` | Ячейка переноса значения между маршрутами | Одно поле без имени и без типа-обёртки: строка либо пусто. Времени жизни между переходами не имеет — обнуляется чтением, а непрочитанный обнуляется teardown ENT-23 (D-46). **Порядок относительно teardown (D-105):** буфер, записанный инициатором **текущего** перехода, teardown не трогает — иначе «Продолжить с этим» ломается на своём же механизме; обнуляется непрочитанный буфер **прошлого** перехода. Роутер владеет обоими и потому упорядочивает их сам (ADR-02). Проверяется тем же автотестом NFR-10, что и поля |
...
| `published` → `withdrawn` | Позиция выведена из каталога | Маршрут **остаётся пожизненно** и отдаёт страницу на весь экран: строка `Инструмент ‹Tool.name› выведен из каталога.` и действие `Открыть палитру` — строка content-guide §5 (D-159); ни причины, ни даты, событий страница не порождает; запись убирается из sitemap, **редиректа нет**. Сохранённый ярлык и `start_url` установленного PWA продолжают открываться и объясняют, что произошло (D-82 п. 3, REQ-CORE-01). **Статус ответа — `200`, страница несёт `noindex`** (D-118) | ROLE-03 |
...
stateDiagram-v2
    ...
    pending --> failed : разбор не состоялся по явному запросу
    pending --> counted : тип не распознан, показаны счётчик ввода и чипсы кодирования (D-137)
    counted --> pending : ввод изменён либо нажат чипс кодирования
    counted --> empty : очищено
...
| `counted` | Счётчик ввода и чипсы кодирования: тип не распознан, но человек видит, что перед ним. **Точка успеха FL-04 и не отказ:** событий не порождает (D-137) | FL-04, D-52 п. 2, D-137 |
...
| `pr` → `merged` | Сборка, нарушающая NFR из состава NFR-17, не проходит; способа выпустить мимо обязательных проверок нет (D-173) | NFR-17, ROLE-06 |
...
| `Result.status` (8) | Перечисление | Экранные состояния, на них ветвится вёрстка (§3.2) | Как данные — состояние без вёрстки |
...
| `failureReason` (6) | Перечисление **с публичным контрактом** | Шесть категорий REQ-METR-02 (D-132, D-133), и значение входит в закрытый список ENT-29 | Особый случай: изменение проходит через страницу «Как это проверить» и гейт CI раньше, чем через код (§7.3) |
...
| `platformClass` (2: `desktop`, `mobile`) × `axis` (3 в P0a) | Перечисления | Ключ порога — тройка «инструмент × класс платформы × ось» (D-73 п. 1); класс определяется при старте оболочки по `(pointer: coarse)` (D-161) | — |
...
Пять событий (EVT-01…EVT-05, D-120) и семь полей закрытого списка (ENT-29) — это **весь** аудитный след
...
Отдельно: PM-01…PM-10 сравниваются с предыдущим релизом, поэтому **изменение смысла поля есть
...
|---|---|---|---|
| ~~Q-138~~ | `expectedChips` фикстуры зависит от состава сборки: при жёстком минимуме M-02 ступени отсутствующих инструментов не активны (D-49 п. 3), и ожидаемый состав чипсов у каждого образца другой. Одно значение в записи корпус описать не может | D-73 п. 3 × D-49 п. 3 | **P2** · **закрыт D-109 2026-09-09** |
| ~~Q-139~~ | `Tool.detectorStep.meaningfulWhen` — предикат, то есть код, но живёт в записи реестра (D-73 п. 3). Правило «добавление позиции не правит оболочку» (REQ-CORE-01, US-002 крит. 1) не сказано, распространяется ли на предикат и где проходит граница между данными реестра и его кодом | D-73 п. 3 × REQ-CORE-01 | **P2** · **закрыт D-109 2026-09-09** |
| ~~Q-140~~ | Страница выведенной позиции (`withdrawn`) обязана существовать по D-82 п. 3, но её текста нет ни в content-guide, ни в требовании; при этом REQ-LIMIT-04 запрещает молчаливый отказ, а страница — ровно случай, где молчание запрещено | D-82 п. 3 × REQ-LIMIT-04 | P3 · **закрыт D-159 2026-09-15** |
| ~~Q-141~~ | `StaticPage.description`: гейт уникальности `title`/`description` сформулирован для лендингов (REQ-CONTENT-03), а состав записи служебной страницы (D-82 п. 1) поля `description` не содержит. Либо гейт у́же, чем реестр, либо состав неполон | D-82 п. 1 × REQ-CONTENT-03 | P3 · **закрыт D-160 2026-09-15** |
| ~~Q-142~~ | `platformClass` — перечисление из двух значений, `desktop` и `mobileSafari`; NFR-14 называет четыре браузера. К какому классу относится мобильный Chrome, модель не выводит, а порог обязан быть у каждой пары «инструмент × класс» | NFR-14 × D-73 п. 1 | P3 · **закрыт D-161 2026-09-15** |
