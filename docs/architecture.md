...
| Наружу | `AnalyticsEvent`: семь полей ENT-29 — `name`, `url`, `referrer`, `screenResolution`, `browser`, `ttr`, `toolId`, `failureReason` | Онлайн, при отсутствии DNT и GPC | Содержимого ввода, результата, ключа, токена, их фрагментов и длин, по которым содержимое восстановимо (REQ-TRUST-06) |
...
| **КМП-07** | Отправитель событий | Отправляет семь разрешённых полей с таймаутом ≤ 1 с и нулём повторов; молчит офлайн, при DNT и при GPC | FL-02 шаг 2, FL-11 A2 |
...
| **КМП-18** | Прокси `/s/*` | Переписывает `/s/script.js` и `/s/event` на VPS, чтобы страница не обращалась к чужому origin | инструментовка всех FL, предъявление FL-13 |
| **КМП-19** | Umami 3 и PostgreSQL 18 | Принимает и агрегирует события; отвечает на восемь прокси-метрик PM-01…PM-09 | ни один FL-NN пользователя — см. ниже |
...
питает решения ROLE-03 — восемь метрик PM-01…PM-09, на которых стоят вехи M-08, M-09 и M-15, — и
...
| **ИНТ-02** | Прокси `/s/*` → Umami | HTTPS POST, same-origin rewrite | Worker бесплатного плана: **100 000 запросов в сутки**, 10 мс CPU на запрос, ≤ 50 подзапросов, 128 MB, старт ≤ 1 с. При исчерпании суточного лимита маршрут настраивается как *fail open* (обход Worker) или *fail closed* (страница ошибки) ([Cloudflare](https://developers.cloudflare.com/workers/platform/limits/)) | Таймаут ≤ 1 с, 0 повторов, деградация молча | **Ничего.** 0 видимых ошибок и 0 мс прироста TTR (NFR-11). Продукт работает полностью; теряются метрики, а не функции |
| **ИНТ-03** | Umami 3 + PostgreSQL 18 на VPS | HTTP внутри прокси | Своя машина, $5–6 в месяц; лимитов провайдера нет, есть лимит диска и памяти | Падение VPS, миграция Umami, исчерпание диска | **Ничего** (тот же NFR-11). Мейнтейнер узнаёт из §8, а не от пользователя |
...
| **ИНТ-09** | Регистратор домена и DNS зоны `dokey.app` | DNS | Зона `.app` целиком в HSTS-preload — HTTP невозможен по построению | Потеря доступа к аккаунту, истечение регистрации | Продукт недоступен целиком, включая сохранённые ярлыки и `start_url` установленного PWA. Второго носителя доступа нет — Q-150 |
...
точка, у которой нет ни второго носителя, ни отката, ни кеша: сохранённый ярлык, `start_url`
установленного PWA и вся выдача D-07 привязаны к имени, а не к содержимому.
...
worker — и вот его последствие ни одним документом пока не разобрано (Q-151).
...
| Признак «подсказка о ярлыке показана» | `localStorage` | К-3 | Бессрочно | **Единственный** служебный ключ закрытого списка D-71; список публикуется на «Как это проверить» |
...
| Событие аналитики | PostgreSQL под Umami | К-4 | «Пока живёт VPS» — срок не назначен, **Q-153** | Не определена |
...
в одну сторону: аналитика деградирует молча.
...
Отсутствие staging — осознанная экономия при О-01 и О-03, но у неё есть один дорогой случай:
**обновление service worker с предыдущей прод-сборки на новую проверяется только на проде.** Preview
на другом origin получает свой scope и свои кеши, то есть проверяет механизм, но не миграцию.
Вопрос вынесен в Q-154; до его закрытия единственная страховка — тридцатидневная жизнь файлов
прошлых сборок и окно версии в подвале.
...
|---|---|---|---|
| **Гейты CI** — основной | Нарушение любого из бюджетов NFR-01…06, 08, 09, 10, 13; регрессию определителя на корпусе фикстур; расхождение sitemap и реестра; исчезнувший `Tool.id` | До релиза | **Предотвращают, а не обнаруживают.** Ничего не говорят о том, что происходит после выпуска |
| **Umami, PM-01…PM-09** | Долю визитов с результатом, категории отказа, долю ручных переключений, полевой TTR, источники входа | Помесячно и пореализно; первое число — 2027-01-20 | Агрегаты по доставленным событиям. Офлайновой части визита в данных не существует |
...
- **Логи хостинга** — у поставщика, ограничены прокси.
- **Логи VPS** — nginx, Umami и PostgreSQL, доступны мейнтейнеру, не мониторятся.
...
| Расписание отключено GitHub за 60 дней без активности репозитория | — | **Нет.** Молча, и внешне неотличимо от предыдущей строки — Q-152 |
| `dokey.app` недоступен | — | **Нет.** Узнаём от пользователя или случайно — Q-156 |
| Приёмник аналитики недоступен | — | **Нет** и не может быть в продукте: NFR-11 требует тишины. Обнаруживается только снаружи — Q-156 |
| VPS исчерпал диск | — | **Нет** — усугубляется отсутствием ретенции, Q-153 |
...
**Предлагаемый минимум, укладывающийся в О-01, О-03 и О-07** (решения нет — Q-156): синтетическая
...
|---|---|---|---|
| **Время мейнтейнера** (О-01) | Два месяца подряд с ручным вмешательством в VPS (триггер ST-04) | Не измерен — Q-145 | Лестница ST-04: перенести Umami на бесплатный serverless → Plausible Cloud → **выключить аналитику вовсе** и жить на лабораторных числах. Продукт не ломается: NFR-11 уже требует молчаливой деградации |
...
| **Диск VPS** под событиями | Свободное место на $5-машине | Не измерен; ретенция не назначена — Q-153 | Назначить ретенцию и удалять старше неё. Метрики PM-01…PM-09 сравниваются с **предыдущим релизом**, длинного ряда им не нужно |
| **Docker Hub: 100 pull за 6 ч на IP** (ИНТ-07) | 429 на шаге сборки | Раннеры GitHub делят адреса — запас не наш и не измерим | Аутентифицироваться в Docker Hub из CI либо зеркалировать базовые образы в GHCR — Q-155 |
| **Расписание Actions** | Дата последней сборки перестала обновляться | — | Разбор Q-152: два выключателя вместо одного |
...
|---|---|---|---|---|
| **ИНВ-01** | Содержимое ввода и результата не покидает вкладку ни при каких условиях. `script-src` и `connect-src` — `'self'` без исключений | REQ-TRUST-01, D-05 | Гейт «сторонних origin = 0», счётчик на первом экране | Любая «тяжёлая конвертация на сервере», любой SDK, любой CDN |
| **ИНВ-02** | 0 байт пользовательского ввода в `localStorage`, `sessionStorage`, URL, query, фрагменте и истории. Служебных ключей — закрытый список, в P0a из одного | NFR-10, D-71 | Автотест нулевой персистентности на каждой сборке | «Сохраним черновик», «восстановим после обновления», ссылка с данными |
...
| **ИНВ-06** | Реестр — единственный источник маршрутов, sitemap, палитры и активных ступеней лестницы | REQ-CORE-01, D-49 | Гейты «sitemap ≡ реестр» и «лестница ≡ реестр» | Маршрут, заведённый файлом; ступень инструмента, которого нет в сборке |
...
|---|---|---|---|
| **Q-151** | Service worker требует защищённого контекста; образ в интранете обычно стоит на `http` — NFR-12 в целевой конфигурации FL-17 недостижим | §4.2, NFR-12 × NFR-16 × D-64 | **P1** |
| **Q-152** | У расписания пересборки два выключателя: наш (три провала) и чужой (GitHub гасит `schedule` в публичном репозитории после 60 дней без активности) | §6, §8.3, D-59 × docs.github.com | P2 |
| **Q-153** | Срок хранения событий в PostgreSQL под Umami не назначен ничем | §5, data-model §0 К-4 × ADR-11 | P2 |
| **Q-154** | Окружения staging нет; обновление SW с прошлой прод-сборки проверяется только на проде | §7.3, ST-07 × D-67 | P2 |
| **Q-155** | CI тянет базовые образы с Docker Hub анонимно с общих адресов раннеров; 429 неотличим от сломанной сборки | §4.1 ИНТ-07, ADR-04 × ADR-14 | P2 |
| **Q-156** | Прод не наблюдается ничем: отказ хостинга и отказ приёмника обнаруживаются пользователем или случайно | §8.3, NFR-11 × О-07 | P2 |
| **Q-157** | Поведение прокси при исчерпании суточного лимита Worker (*fail open* или *fail closed*) не выбрано; это настройка провайдера с последствием для NFR-11 | §4.1 ИНТ-02, ADR-10 × NFR-11 | P3 |
| **Q-158** | Заголовки ответа существуют в двух копиях — `_headers` хостинга и конфигурация nginx в образе; равенство не проверяется ничем | §7, TECH-02 × TECH-09 × ST-03 | P2 |
