...
flowchart LR
  ...
  subgraph THIRD["Внешние поставщики"]
    ...
    REG["Регистратор и DNS<br/>зона dokey.ru"]
  end
...
| **ИНТ-09** | Регистратор домена и DNS зоны `dokey.ru` | DNS | HTTP закрыт редиректом и HSTS; зона `.ru` в HSTS-preload не входит, домен подаётся в список сам — T-223 (D-175) | Потеря доступа к аккаунту, истечение регистрации | Продукт недоступен целиком, включая сохранённые ярлыки и `start_url` установленного PWA. Регистратор — не Cloudflare, домен продлевается по году с автопродлением (SEC-12, D-177); при потере Cloudflare меняются NS у регистратора и включается ST-03. Второй носитель — отложенный аварийный доступ доверенного лица к набору восстановления (D-162) |
...
(D-162): регистратор — не Cloudflare, домен продлевается по году с автопродлением (SEC-12, D-177), при потере Cloudflare
...
| **prod** | `dokey.ru` на Cloudflare Workers + статические ассеты | Всё остальное | Только конвейер: деплой из Actions через wrangler |
...
Окружений стало четыре: **staging заведён D-111** на `stage.dokey.ru` — тот же артефакт, профиль
...
| `dokey.ru` недоступен | Синтетическая проверка каждые 6 часов, issue после двух неуспехов подряд (D-108) | **Да** с T-031 |
...
проверка в том же расписании Actions — `curl` на `dokey.ru` и на путь прокси, при неуспехе
...
|---|---|---|---|---|
| **ИНВ-01** | Содержимое ввода и результата не покидает вкладку ни при каких условиях. `script-src` и `connect-src` — `'self'` без исключений. Сторонний origin — и адрес отчётов, объявленный заголовком ответа (`Report-To`, `Reporting-Endpoints`, `NEL`, `report-uri` и `report-to` в CSP), кто бы его ни добавил: мы или хостинг (D-180) | REQ-TRUST-01, D-05, D-180 | Гейт G-04 в двух условиях (D-95): «сторонних origin = 0» **и** положительная проверка — подстрока фикстуры не встречается ни в одном URL, заголовке и теле запроса, **независимо от origin**; RUN-16 на проде: в ответе нет `nel`, `report-to`, `reporting-endpoints` (D-180); счётчик на первом экране | Любая «тяжёлая конвертация на сервере», любой SDK, любой CDN, включённый у хостинга сбор отчётов браузера |
