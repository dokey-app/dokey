...
- [ ] T-239 — привести путь ЭП-06 к факту (⚠ Q-246): api-spec §4.6 и `openapi.yaml` называют хешированные ассеты `/assets/{name}.{hash}.{ext}`, а сборка Astro и оба файла заголовков отдают их из `/_astro/*` (найдено в /check T-234 2026-09-22): границы — `docs/api-spec.md`, `docs/openapi.yaml` (перегенерация, не правка руками); готово — ни api-spec, ни `openapi.yaml` не содержат `/assets/`, путь ЭП-06 совпадает с правилом `infra/headers/_headers`
