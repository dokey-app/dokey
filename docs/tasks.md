...
- [ ] T-236 — уложить `tests/unit/docs-structure.test.ts` в таймаут Vitest: каждый из двух случаев заново читает все 80 файлов `docs/**/*.md` (4,3 МБ) и идёт 7–8 с при пороге 5 с, поэтому `pnpm test:unit` красный на чистом дереве, и падает то один случай, то другой (найдено в T-235 2026-09-18, Windows): границы — `tests/unit/docs-structure.test.ts`; готово — `pnpm test:unit` зелёный три прогона подряд; блок: T-237
- [x] T-237 — уложить в таймаут Vitest два остальных случая `tests/unit`: `size-gate.test.ts` и `check-headers.test.ts` поднимают на случай по дочернему процессу и при загруженной машине выходят за 5 с — падает то один случай, то другой, а на свободной машине оба зелёные (найдено в T-236 2026-09-20): границы — `tests/unit/size-gate.test.ts`, `tests/unit/check-headers.test.ts`; готово — `pnpm test:unit` зелёный три прогона подряд при занятых ядрах
...
- 2026-09-20 T-237 `2e84902` — случаи `tests/unit` с дочерним процессом уложены в таймаут Vitest: работа один раз на файл и параллельно, `spawnSync` заменён асинхронным `execFile`; 51 из 51 зелёный при занятых ядрах
