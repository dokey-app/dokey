...
- [ ] T-253 — баг: `pnpm format:check` красный на чистом дереве: Prettier обходит неотслеживаемые каталоги обвязки (`orchestrator/`, `.orchestrator/`, `.claude/commands/orchestrate.md`), исключённые через `.git/info/exclude`, но не через `.prettierignore`, и пункт 1 чек-листа `.github/pull_request_template.md` не проходит ни у одной задачи, хотя в CI и по `git ls-files -z | xargs -0 -n 80 prettier --check --ignore-unknown` всё зелёное (найдено в T-224, T-244, T-249, T-251): границы — `.prettierignore` либо формулировка пункта чек-листа; готово — `pnpm format:check` зелёный на чистом дереве с этими каталогами, состав проверок CI не меняется
