...
| **Э-2** | ~~Q-212, Q-213~~ (D-102), ~~Q-138, Q-139~~ (D-109) — все закрыты | Оба P1 были про бюджет и дату; Э-2 — первая встреча плана с календарём |
...
| **Релиз** | ~~Q-206~~ (D-143), Q-209, Q-210, ~~Q-214~~ (D-145) | Из четырёх про допуск релиза открыты два: что блокирует и состав NFR в глоссарии против PRD |
...
- [ ] T-011 — задеплоить заглушку на `dokey.app` через wrangler (TECH-02, ADR-10, D-114): границы — `wrangler.toml`, `.github/workflows/release.yml`; готово — страница открывается, `curl -sI https://dokey.app` возвращает ответ хостинга и `strict-transport-security`, заголовки совпадают с `_headers`
...
- [ ] T-031 — завести синтетическую проверку прода каждые 6 часов (D-108): границы — `.github/workflows/synthetic.yml`, `scripts/check-prod.ts`; готово — `pnpm check:prod` проверяет пять утверждений (прод отвечает `200` с заголовками из `policy.ts`; `GET /s/event` — `405`; `GET /s/script.js` — `404`; `/s/recorder.js` — `404`; приёмник принимает `POST`), два неуспеха подряд заводят issue, третий issue не заводится, пока открыт предыдущий
