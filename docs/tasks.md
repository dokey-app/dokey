...
- [ ] T-240 — не выдавать «пусто» за «проверено» в проверке склейки `gate:headers-parity`: если в `infra/headers/_headers` нет правила `/*`, `concatenated()` возвращает пустой список, и гейт печатает «частные правила … заменяют заголовки /*» (найдено в /check T-234 2026-09-22): границы — `scripts/gates/headers-parity.ts`; готово — `_headers` без правила `/*` даёт `fail` с объяснением, на текущем файле гейт зелёный
