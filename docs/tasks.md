...
- [ ] T-011 — задеплоить заглушку на `dokey.app` через wrangler (TECH-02, ADR-10, D-114, D-162): границы — `wrangler.toml`, `.github/workflows/release.yml`; готово — страница открывается, `curl -sI https://dokey.app` возвращает ответ хостинга и `strict-transport-security`, заголовки совпадают с `_headers`, токен деплоя — со сроком ≤ 6 месяцев и одним правом `Workers Scripts:Edit`
- [ ] T-012 — закрыть индексацию до тега релиза: `Disallow: /` и `noindex` (D-85): границы — `public/robots.txt`, `src/layouts/Base.astro`; готово — `curl https://dokey.app/robots.txt` отдаёт запрет обхода
...
- [ ] T-207 — завести корневой маршрут `/` записью реестра: оболочка с открытой палитрой, цель `start_url` (US-014, D-91, D-82 п. 1): границы — `src/registry/pages.ts`, `src/pages/index.astro`; готово — `curl https://dokey.app/` отдаёт оболочку, `pnpm test:e2e -t root-palette`: вход из PWA открывает палитру
...
- [ ] T-156 — поднять staging `stage.dokey.app` с профилем без аналитики, `noindex` и `Disallow: /` (D-111): границы — `wrangler.toml`, `.github/workflows/pr.yml`; готово — сборка PR по метке открывается по адресу staging, `noindex` на месте, событий в Umami ноль
...
- [ ] T-201 — открыть индексацию: переключить признак индексации сборки, из которого выводятся `noindex` и `robots.txt` (обратная к T-012, D-164): границы — `astro.config.mjs`; готово — `curl https://dokey.app/robots.txt` отдаёт `Disallow: /s/` и `Sitemap:`, `noindex` на страницах нет
