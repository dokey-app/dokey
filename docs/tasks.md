...
- [ ] T-031 — завести синтетическую проверку прода каждые 6 часов (D-108, D-167): границы — `.github/workflows/synthetic.yml`, `scripts/check-prod.ts`; готово — `pnpm check:prod` проверяет пять утверждений (прод отвечает `200` с заголовками из `policy.ts`; `GET /s/event` — `405`; `GET /s/script.js` — `404`; `/s/recorder.js` — `404`; приёмник принимает `POST`), два неуспеха подряд заводят issue, третий issue не заводится, пока открыт предыдущий, ответ `/s/event` без конверта §3.2 при живом проде заводит issue «Worker не исполняется: суточный лимит (сброс в 00:00 UTC) или снят маршрут»
